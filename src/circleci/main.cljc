(ns circleci.main
  "Kotodama WASM entrypoint for the Circleci clean-room actor (L5) — Clojure port.
  ...
  facts (`circleci.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "circleci")
(def tier "L5")
...
(def entity-specs
  [{:entity "Pipeline"  :plural "pipelines"  :id-prefix "circleci_pip"
    :fields [:number :state :projectSlug]                :required [:number]
    :coerce {:number :int}                   :refs {}}
   {:entity "Workflow"  :plural "workflows"  :id-prefix "circleci_wor"
    :fields [:pipelineId :name :status :startedBy :stoppedAt] :required [:name]
    :coerce {}                                :refs {:pipelineId "Pipeline"}}
   {:entity "Job"       :plural "jobs"       :id-prefix "circleci_job"
    :fields [:jobNumber :name :status :duration :workflowId :stoppedAt] :required [:jobNumber]
    :coerce {:jobNumber :int :duration :int} :refs {:workflowId "Workflow"}}
   {:entity "Project"   :plural "projects"   :id-prefix "circleci_pro"
    :fields [:slug :name :externalUrl]       :required [:slug]
    :coerce {}                                :refs {}}
   {:entity "User"      :plural "users"      :id-prefix "circleci_use"
    :fields [:login :name :avatarUrl]        :required [:login]
    :coerce {}                                :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"circleci.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "circleci-compat" :tier tier :entities entities} 200])
...
(defn start! [] :circleci-compat/ready)
